...
void test_personalized_page_rank(int nr, int nc, int c, double q, unsigned int seed, int pr, int pc, int NR, int NC, double beta, double epsilon, int max_iterations, std::vector<int> personalized_nodes) {
  int world, rank;
...
void test_mat(int nr, int nc, int c, double beta, double epsilon, int max_iterations, std::vector<int> personalized_nodes) {
  for(double s = 0; s < 4; s++) {
    for(double q = 0; q <= 1; q += 0.1) {
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      test_personalized_page_rank(nr, nc, c, q, s, rank, 0, world, 1, beta, epsilon, max_iterations, personalized_nodes);
      MPI_Barrier(MPI_COMM_WORLD);
      if (rank == 0)
        std::cout << "--- col ---- nr : " << nr << "; nc : " << nc << "; c : " << q << ": " << q << "; s : " << s << "----" << std::endl;
      MPI_Barrier(MPI_COMM_WORLD);
      test_personalized_page_rank(nr, nc, c, q, s, 0, rank, 1, world, beta, epsilon, max_iterations,personalized_nodes);
      MPI_Barrier(MPI_COMM_WORLD);
      if(world % 2 == 0 && world / 2 > 1) {
        if (rank == 0)
          std::cout << "--- mix ---- nr : " << nr << "; nc : " << nc << "; c : " << q << ": " << q << "; s : " << s << "----" << std::endl;
        MPI_Barrier(MPI_COMM_WORLD);
        test_personalized_page_rank(nr, nc, c, q, s, rank / 2, rank % 2, world / 2, 2, beta, epsilon, max_iterations,personalized_nodes);
        MPI_Barrier(MPI_COMM_WORLD);
      }
    }
  }
